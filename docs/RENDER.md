...
3. Deploy the Blueprint. Render builds the Docker image, runs `alembic upgrade head` as the pre-deploy command, then starts the application. The Docker start command also runs the same migrations before opening the web server, so manual Web Services work even without a pre-deploy hook. The health check is `/healthz`.
...
| Migrations | Pre-deploy and Docker startup both run Alembic upgrade; already-applied revisions are skipped; a failure stops startup |
...
- **Missing tables / `relation "accounts" does not exist`:** Deploy the latest `main` commit. Leave **Docker Command** blank (uses the Dockerfile) or set it to `python -m optionlab.server`. This applies migrations before startup. A custom direct `uvicorn` command bypasses this step. Keep `alembic upgrade head` as the optional pre-deploy command. Both steps must use the same `DATABASE_URL`.
...
A dedicated Supabase PostgreSQL project can replace Render Postgres. For manual services, set `DATABASE_URL` to the **Session pooler** URI from Supabase Connect (port 5432), with the database password URL-encoded and `sslmode=require`. Use session mode for the migration runner’s PostgreSQL advisory lock. Disable Supabase’s Data API for this backend-only project, since these migrations do not configure Supabase RLS policies. The existing Blueprint provisions Render Postgres; use manual service configuration when supplying Supabase instead.
